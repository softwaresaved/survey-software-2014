...
df <-read.csv("SurveyDataCleaned.csv", stringsAsFactors=TRUE, na.strings="")
```
...
##2.1 Habits concerning software in research 
...
```{r fig.width = 20, fig.height=10, echo=FALSE}
	sumQ  <-  singleTabFreq(df$Q.6.Use_software, 'Use of Software')
	kable(sumQ, digits=2)
...
dfYES <- df[which(df$Q.6.Use_software=='Yes'),]
dfYES <- droplevels(dfYES)
```

## 2.1.1. Importance of software
...
```{r echo=FALSE}
  levels(dfYES$Q.7.Importance_software)[1] <- "No Difference"
  levels(dfYES$Q.7.Importance_software)[2] <- "Not be Practical"
  levels(dfYES$Q.7.Importance_software)[3] <- "More effort"

```

```{r fig.width = 20, fig.height=10, echo=FALSE}
	sumQ  <-  singleTabFreq(dfYES$Q.7.Importance_software, 'Importance of Software')
	kable(sumQ, digits=2)
...
```{r echo=FALSE}
	...
	dfYES$Q.7.Essential_software <- revalue(dfYES$Q.7.Importance_software, c('No Difference'='No-Essential', 'More effort'='No-Essential', 'Not be Practical'='Essential'))
```

```{r fig.width=20, fig.height=10, echo=FALSE}

	sumQ  <-  singleTabFreq(dfYES$Q.7.Essential_software, 'Software Essential for research')
	kable(sumQ, digits=2)
	plotSingleFreq(sumQ, 'Software Essential for research')
```

## 2.1.2. Developing its own research software.
...
```{r fig.width=20, fig.height=10, echo=FALSE}
	...
	sumQ  <-  singleTabFreq(dfYES$Q.8.Developing_software, 'Developing Software')
	kable(sumQ, digits=2)
	plotSingleFreq(sumQ, 'Developing Software')
```

### 2.1.3. Training to develop software


```{r fig.width=20, fig.height=10, echo=FALSE}

	sumQ  <-  singleTabFreq(dfYES$Q.9.Clean, 'Training in Software development')
	kable(sumQ, digits=2)
...
### 2.1.4. Training versus developing


```{r fig.width=20, fig.height=10, echo=FALSE}

	sumQ  <-  singleTabFreq(dfYES[which(dfYES$Q.8.Developing_software == 'Yes'),]$Q.9.Clean, 'Training in Software development')
	kable(sumQ, digits=2)
	plotSingleFreq(sumQ, 'Training in Software development', ownColour=color_Developing)
...
```{r echo=FALSE}
    levels(factor(dfYES$Q.3.Discipline))
```
...
```{r echo=FALSE}
    ...
    dfYES$Q.3.Modified_disciplines <- dfYES$Q.3.Discipline
    dfYES$Q.3.Modified_disciplines <- recode(dfYES$Q.3.Modified_disciplines, "DesArHu='Design+Art+Hum'")
    dfYES$Q.3.Modified_disciplines <- recode(dfYES$Q.3.Modified_disciplines, "AdmSoEd='Admin+Social+Edu'")
    dfYES$Q.3.Modified_disciplines <- recode(dfYES$Q.3.Modified_disciplines, "MedAgri='Medicine+Agri'")

```
Then the distribution table seems more usable even if we probably make more academics angry at this point. 

```{r fig.width=20, fig.height=10, echo=FALSE}
    modifiedQ3 <- singleTabFreq(dfYES$Q.3.Modified_disciplines, 'Modified Disciplines')
    #Print table
...
### 3.1.2. Importance of software per disciplines

```{r fig.width=20, fig.height=10, echo=FALSE}
	sumQ  <- crossTabFreq(dfYES, 'Q.3.Modified_disciplines', 'Q.7.Essential_software') 
	facetPlot(sumQ, facetVar='Q.3.Modified_disciplines', xVar='Q.7.Essential_software')

```

### 3.1.2. Developing software per disciplines

```{r fig.width=20, fig.height=10, echo=FALSE}
	sumQ  <- crossTabFreq(dfYES, 'Q.3.Modified_disciplines', 'Q.8.Developing_software') 
	facetPlot(sumQ, facetVar='Q.3.Modified_disciplines', xVar='Q.8.Developing_software')

```

### 3.1.3. Developing software per disciplines

```{r fig.width=20, fig.height=10, echo=FALSE}
	sumQ  <- crossTabFreq(dfYES, 'Q.3.Modified_disciplines', 'Q.9.Clean') 
	facetPlot(sumQ, facetVar='Q.3.Modified_disciplines', xVar='Q.9.Clean')
...
```{r echo=FALSE}
    levels(factor(dfYES$Q.4.Funds_cleaned))
```
...
```{r fig.width=20, fig.height=10, echo=FALSE}
    sumQ <- singleTabFreq(dfYES$Q.4.Funds_cleaned, 'Funds')
...
```{r, echo=FALSE}
    dfYES$Q.4.Modified_Funds <- dfYES$Q.4.Funds_cleaned
    levels(dfYES$Q.4.Modified_Funds)[c(3, 8, 13, 17)] <- 'None'
    levels(dfYES$Q.4.Modified_Funds)[c(8, 13, 17)] <- 'Charity/Trust'
    levels(dfYES$Q.4.Modified_Funds)[c(1, 2, 4, 5, 9, 11, 14)] <- 'Research Council'
    levels(dfYES$Q.4.Modified_Funds)[c(6,8)] <- 'Other UK'
    levels(dfYES$Q.4.Modified_Funds)[c(3, 7)] <- 'Other Non-UK'
```


```{r fig.width=20, fig.height=10, echo=FALSE}
    sumQ <- singleTabFreq(dfYES$Q.4.Modified_Funds,'Modified Funds')
    #Printing table
    kable(sumQ, digits=2, row.names = FALSE)
...
### 3.2.1. Importance of software per funding origin

```{r fig.width=20, fig.height=10, echo=FALSE}
	sumQ  <- crossTabFreq(dfYES, 'Q.4.Modified_Funds', 'Q.7.Essential_software') 
	facetPlot(sumQ, facetVar='Q.4.Modified_Funds', xVar='Q.7.Essential_software')

```

### 3.2.2. Developing software per funding origin 
```{r fig.width=20, fig.height=10, echo=FALSE}
	sumQ  <- crossTabFreq(dfYES, 'Q.4.Modified_Funds', 'Q.8.Developing_software') 
	facetPlot(sumQ, facetVar='Q.4.Modified_Funds', xVar='Q.8.Developing_software')

```

### 3.2.3. Training software per funding origin 
```{r fig.width=20, fig.height=10, echo=FALSE}
	sumQ  <- crossTabFreq(dfYES, 'Q.4.Modified_Funds', 'Q.9.Clean') 
	facetPlot(sumQ, facetVar='Q.4.Modified_Funds', xVar='Q.9.Clean')
...
### 3.3.1. Importance of software per Seniority

```{r fig.width=20, fig.height=10, echo=FALSE}
	sumQ  <- crossTabFreq(dfYES, 'Q.5.Seniority', 'Q.7.Essential_software') 
	facetPlot(sumQ, facetVar='Q.5.Seniority', xVar='Q.7.Essential_software')
```

### 3.3.2. Developing software per Seniority 
```{r fig.width=20, fig.height=10, echo=FALSE}
	sumQ  <- crossTabFreq(dfYES, 'Q.5.Seniority', 'Q.8.Developing_software') 
	facetPlot(sumQ, facetVar='Q.5.Seniority', xVar='Q.8.Developing_software')
```

### 3.3.3. Training software per Seniority 
```{r fig.width=20, fig.height=10, echo=FALSE}
	sumQ  <- crossTabFreq(dfYES, 'Q.5.Seniority', 'Q.9.Clean') 
	facetPlot(sumQ, facetVar='Q.5.Seniority', xVar='Q.9.Clean')
```

## 3.4. Job title

```{r fig.width=20, fig.height=10, echo=FALSE}
    sumQ <- singleTabFreq(dfYES$E.1.Job_cleaned, 'Job Title')
    #Printing table
    kable(sumQ, digits=2)
    # Printing Plot
...
### 3.4.1. Importance of software per Job Title

```{r fig.width=20, fig.height=10, echo=FALSE}
	sumQ  <- crossTabFreq(dfYES, 'E.1.Job_cleaned', 'Q.7.Essential_software') 
	facetPlot(sumQ, facetVar='E.1.Job_cleaned', xVar='Q.7.Essential_software')
```

### 3.4.2. Developing software per Job Title

```{r fig.width=20, fig.height=10, echo=FALSE}
	sumQ  <- crossTabFreq(dfYES, 'E.1.Job_cleaned', 'Q.8.Developing_software') 
	facetPlot(sumQ, facetVar='E.1.Job_cleaned', xVar='Q.8.Developing_software')
```

### 3.4.3. Training software per Job Title

```{r fig.width=20, fig.height=10, echo=FALSE}
	sumQ  <- crossTabFreq(dfYES, 'E.1.Job_cleaned', 'Q.9.Clean') 
	facetPlot(sumQ, facetVar='E.1.Job_cleaned', xVar='Q.9.Clean')


## 3.5. Gender

```{r fig.width=20, fig.height=10, echo=FALSE}
    sumQ <- singleTabFreq(dfYES$E.2.GenderCleaned, 'Gender')
    #Printing table
    kable(sumQ, digits=2)
    # Printing Plot
...
### 3.5.1. Importance of software per Gender

```{r fig.width=20, fig.height=10, echo=FALSE}
	sumQ  <- crossTabFreq(dfYES, 'E.2.GenderCleaned', 'Q.7.Essential_software') 
	facetPlot(sumQ, facetVar='E.2.GenderCleaned', xVar='Q.7.Essential_software')
```

### 3.5.2. Developing software per Gender

```{r fig.width=20, fig.height=10, echo=FALSE}
	sumQ  <- crossTabFreq(dfYES, 'E.2.GenderCleaned', 'Q.8.Developing_software') 
	facetPlot(sumQ, facetVar='E.2.GenderCleaned', xVar='Q.8.Developing_software')
```

### 3.5.3. Training software per Gender

```{r fig.width=20, fig.height=10, echo=FALSE}
	sumQ  <- crossTabFreq(dfYES, 'E.2.GenderCleaned', 'Q.9.Clean') 
	facetPlot(sumQ, facetVar='E.2.GenderCleaned', xVar='Q.9.Clean')
...
```{r fig.width=20, fig.height=10, echo=FALSE}
    sumQ <- singleTabFreq(dfYES$E.4.OS_cleaned, 'Operating System')
...
### 3.6.1. Importance of software per Operating System

```{r fig.width=20, fig.height=10, echo=FALSE}
	sumQ  <- crossTabFreq(dfYES, 'E.4.OS_cleaned', 'Q.7.Essential_software') 
	facetPlot(sumQ, facetVar='E.4.OS_cleaned', xVar='Q.7.Essential_software')
```

### 3.6.2. Developing software per Operating System

```{r fig.width=20, fig.height=10, echo=FALSE}
	sumQ  <- crossTabFreq(dfYES, 'E.4.OS_cleaned', 'Q.8.Developing_software') 
	facetPlot(sumQ, facetVar='E.4.OS_cleaned', xVar='Q.8.Developing_software')
```

### 3.6.3. Training software per Operating System

```{r fig.width=20, fig.height=10, echo=FALSE}
	sumQ  <- crossTabFreq(dfYES, 'E.4.OS_cleaned', 'Q.9.Clean') 
	facetPlot(sumQ, facetVar='E.4.OS_cleaned', xVar='Q.9.Clean')
